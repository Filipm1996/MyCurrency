...
class CryptoViewModel@Inject constructor(
    ...
    fun getRecordFromCoinGeckoByDate(date : LocalDate, currency: Currency) =
        viewModelScope.launch {
            val unixFrom = date.minusDays(1).atStartOfDay(ZoneId.systemDefault()).toInstant().epochSecond
            val unixTo = date.atStartOfDay(ZoneId.systemDefault()).toInstant().epochSecond
            val response = networkRepository.getSingleRecordFromCoinGecko(currency,unixFrom,unixTo)
            when(response){
